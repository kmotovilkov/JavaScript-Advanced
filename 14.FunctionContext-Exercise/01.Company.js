class Company {
    ...
    addEmployee(username, salary, position, department) {
        ...
        let currentDepartment = this.departments.find(dep => dep.name === department);
    ...
    bestDepartment() {
        const depatments = this.departments.map(d => {
            const dep = Object.assign({}, d);
            dep.averageSalary =
                d.employees.reduce((p, c, i, a) => p + c.salary, 0) / d.employees.length;
            return dep;
        });
        depatments.sort((a, b) => b.averageSalary - a.averageSalary);
        const best = depatments[0];

        if (best !== undefined) {
            best.employees.sort((a, b) => b.salary - a.salary || a.username.localeCompare(b.username));
        }

        const result = [`Best Department is: ${best.name}`, `Average salary: ${best.averageSalary.toFixed(2)}`,];
