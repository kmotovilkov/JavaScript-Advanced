function solve() {
    ...
    class ShortReports extends Article {
        ...
        toString() {
            const result = [
                super.toString(), `Original Research: ${this.originalResearch.title} by ${this.originalResearch.author}`
            ];

            if (this.comment.length > 0) {
                result.push('Comments:');
                this.comment.forEach(c => result.push(c));
            }
            return result.join('\n');
        }
    }
    ...
    class BookReview extends Article {
        ...
        addClient(clientName, orderDescription) {
            if (this.clients.find(c => c.className === clientName) !== undefined) {
